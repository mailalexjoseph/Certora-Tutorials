methods{
	// getStateById(uint256 meetingId) returns (MeetingStatus)
	getStartTimeById(uint256 meetingId) returns (uint256)
	getEndTimeById(uint256 meetingId) returns (uint256)
	getNumOfParticipents(uint256 meetingId) returns (uint256)
...
rule monotonousIncreasingNumOfParticipants(method f, uint256 meetingId) {
	env e;
	calldataarg args;
    require getStateById(e, meetingId) == UNINITIALIZED() => getNumOfParticipents(e, meetingId) == UNINITIALIZED();
