 ...
 */
// @note definitions for meeting states
        definition UNINITIALIZED(uint256 id) returns bool = getStartTimeById(id)==0 && getEndTimeById(id)==0 && getNumOfParticipents(id)==0 && getOrganizer(id)==0 && getStateById(id)==0;
        definition PENDING(uint256 id) returns uint8 = getStartTimeById(id)>0 && getEndTimeById(id)>getEndTimeById(id) && getNumOfParticipents(id)==0 && getOrganizer(id)!=0 && getStateById(id)==1;
        definition STARTED(env e, uint256) returns uint8 = getStartTimeById(id)=<e.block.timestamp && getEndTimeById(id)>getEndTimeById(id) && getNumOfParticipents(id)>=0 && getOrganizer(id)!=0 && getStateById(id)==2;
        definition ENDED(env e, uint256 id) returns uint8 = getStartTimeById(id)<e.block.timestamp && getEndTimeById(id)<e.block.timestamp && getNumOfParticipents(id)>=0 && getOrganizer(id)!=0 && getStateById(id)==3;
        definition CANCELLED() returns uint8 = getStartTimeById(id)>0 && getEndTimeById(id)>getEndTimeById(id) && getNumOfParticipents(id)==0 && getOrganizer(id)!=0 && getStateById(id)==4;
...
rule monotonousIncreasingNumOfParticipants(method f, uint256 meetingId) {
	env e;
	calldataarg args;
    require getStateById(meetingId) == UNINITIALIZED() => getNumOfParticipents(meetingId) == UNINITIALIZED();
