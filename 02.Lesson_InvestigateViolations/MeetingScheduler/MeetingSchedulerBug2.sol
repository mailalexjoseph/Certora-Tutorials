...
contract MeetingScheduler is IMeetingScheduler {
    ...
    {
        // @note need to check if the meetingId is initialized to prevent returning garbage data
        require(meetings[meetingId].status !=MeetingStatus.UNINITIALIZED,"Meeting uninitialized");
    ...
    function cancelMeeting(uint256 meetingId) external override {
        ...
        require(
            // @note meeting can be cancelled only if it's in the pending state
            // scheduledMeeting.status == MeetingStatus.STARTED,
            scheduledMeeting.status == MeetingStatus.PENDING,
            // "can't end a meeting if not started"
            "Meeting can be cancelled only it's in the pending state."
        );
