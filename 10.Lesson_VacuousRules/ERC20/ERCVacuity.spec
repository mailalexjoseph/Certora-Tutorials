...
rule twoBalancesGreaterThanSingleRuleCorrected(address account1, address account2){
    assert (balanceOf(account1) + balanceOf(account2) >= balanceOf(account1));
}
...
rule lastRevertedExample(address sender, address recipiecnt, uint256 amount){
    ...
    uint256 _allownce = allowance(sender, e.msg.sender);
    uint256 senderBalance = balanceOf(sender);
    uint256 recipientBalance = balanceOf(recipiecnt);
    transferFrom@withrevert(e, sender, recipiecnt, amount);
    bool success = !lastReverted;
    uint256 allownce_ = allowance(sender, e.msg.sender);

    // assert success => _allownce < amount;
    assert (!success && e.msg.value == 0 && senderBalance>=amount && (senderBalance-amount<senderBalance) && (recipientBalance + amount>recipientBalance))=> _allownce < amount;
}
...
rule ownerChange(address currentOwner, address user){
    ...
    // assert false;
    // @note there is no function in the contract which is changing the _owner. 
    // The first part of assert (ownerAfter!=currentOwner) will always be true
    // precisely because the owner will never change.
...
rule balanceOfChange(method f, address user1, address user2) {
    ...
    // @note this will never be satisfied ((p<q)&&(q<p))
